      ...
      <header className="relative z-10 w-full bg-[#4f252a] border-b border-white/10 shadow-sm">
        <div className="w-full px-4 sm:px-8 lg:px-10 h-[72px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <Link href="/login" className="bg-[#f1745e] hover:bg-[#df624f] text-white text-xs sm:text-sm px-4 sm:px-6 py-2 rounded-lg font-semibold shadow-sm transition">
              Login
            </Link>
          </div>
        </div>
          ...
          >
            ...
            <div className="mt-6 sm:mt-7 space-y-3">
              <div className="rounded-2xl border bg-white/55 px-5 py-4" style={{ borderColor: "rgba(79,37,42,0.14)" }}>
                <div className="flex items-center gap-3 text-[#4f252a] font-bold text-sm sm:text-base"><span className="text-lg">✨</span> Simple & calming</div>
                <p className="mt-1 text-[#4f252a]/75 text-xs sm:text-sm">Clean UI to help you focus on writing.</p>
              </div>
              <div className="rounded-2xl border bg-white/55 px-5 py-4" style={{ borderColor: "rgba(79,37,42,0.14)" }}>
                <div className="flex items-center gap-3 text-[#4f252a] font-bold text-sm sm:text-base"><span className="text-lg">🔒</span> Private by design</div>
                <p className="mt-1 text-[#4f252a]/75 text-xs sm:text-sm">Your journal stays personal and secure.</p>
              </div>
              <div className="rounded-2xl border bg-white/55 px-5 py-4" style={{ borderColor: "rgba(79,37,42,0.14)" }}>
                <div className="flex items-center gap-3 text-[#4f252a] font-bold text-sm sm:text-base"><span className="text-lg">📱</span> Use anywhere</div>
          ...
          <section className="relative flex-1 overflow-hidden">
            ...
            <div className="relative z-10 h-full flex items-center justify-center px-4 sm:px-8 py-8 lg:py-10">
              <div className="w-full max-w-xl lg:max-w-[680px]">
                <h1 className="text-4xl sm:text-5xl font-extrabold text-[#4f252a] text-center mb-5 sm:mb-6">Sign Up</h1>
                ...
                <div className="bg-[#fffaf4]/95 backdrop-blur border rounded-[32px] p-6 sm:p-8 shadow-[0_28px_70px_rgba(79,37,42,0.10)]" style={{ borderColor: "rgba(79,37,42,0.14)" }}>
                  <p className="text-center text-[#4f252a]/70 mb-5 sm:mb-6 text-sm sm:text-base">Please enter your details below</p>

                  {message && <div className="mb-4 rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-sm sm:text-base text-green-700">{message}</div>}
                  ...
                  <form onSubmit={onSubmit} className="space-y-4 sm:space-y-5">
                    <div>
                      <label className="block text-sm font-bold mb-2 text-[#4f252a]">Full Name</label>
                      <input type="text" placeholder="Enter your full name" value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full border rounded-2xl px-4 sm:px-5 py-3 text-sm sm:text-base outline-none focus:ring-2 focus:ring-[#f1745e]/30 bg-white" style={{ borderColor: "rgba(79,37,42,0.14)" }} required />
                    </div>
                    <div>
                      <label className="block text-sm font-bold mb-2 text-[#4f252a]">Email</label>
                      <input type="email" placeholder="Enter your email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border rounded-2xl px-4 sm:px-5 py-3 text-sm sm:text-base outline-none focus:ring-2 focus:ring-[#f1745e]/30 bg-white" style={{ borderColor: "rgba(79,37,42,0.14)" }} required />
                    </div>
                    <div>
                      <label className="block text-sm font-bold mb-2 text-[#4f252a]">Password</label>
                      <div className="relative">
                        <input type={showPassword ? "text" : "password"} placeholder="Create a password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full border rounded-2xl px-4 sm:px-5 py-3 pr-12 sm:pr-14 text-sm sm:text-base outline-none focus:ring-2 focus:ring-[#f1745e]/30 bg-white" style={{ borderColor: "rgba(79,37,42,0.14)" }} required />
                        <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-4 sm:right-5 top-1/2 -translate-y-1/2 text-base sm:text-lg opacity-80 hover:opacity-100 transition" aria-label={showPassword ? "Hide password" : "Show password"}>
                          {showPassword ? "🙈" : "👁️"}
                        </button>
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-bold mb-2 text-[#4f252a]">Confirm Password</label>
                      <div className="relative">
                        <input type={showConfirmPassword ? "text" : "password"} placeholder="Confirm your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full border rounded-2xl px-4 sm:px-5 py-3 pr-12 sm:pr-14 text-sm sm:text-base outline-none focus:ring-2 focus:ring-[#f1745e]/30 bg-white" style={{ borderColor: "rgba(79,37,42,0.14)" }} required />
                        <button type="button" onClick={() => setShowConfirmPassword((v) => !v)} className="absolute right-4 sm:right-5 top-1/2 -translate-y-1/2 text-base sm:text-lg opacity-80 hover:opacity-100 transition" aria-label={showConfirmPassword ? "Hide confirm password" : "Show confirm password"}>
                    ...
                    <div className="flex justify-center pt-2">
                      <button type="submit" disabled={loading} className="w-full sm:w-auto sm:px-20 px-8 py-3 rounded-2xl text-sm sm:text-base font-extrabold text-white shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition disabled:opacity-60" style={{ background: "linear-gradient(90deg, #f1745e 0%, #df624f 100%)" }}>
                        {loading ? "Creating..." : "Create Account"}
                      </button>
                    </div>

                    <p className="text-center text-xs sm:text-sm text-[#4f252a]/70 pt-1">
                      Already have an account? <Link href="/login" className="text-[#f1745e] underline font-extrabold">Log In</Link>
                    </p>
                  </form>
                </div>
